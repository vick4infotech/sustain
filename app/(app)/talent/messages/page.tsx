...
export default async function TalentMessagesPage({
  ...
  return (
    <div>
      <h1>Messages</h1>
      <p className="muted">Communicate with the programme team, and with companies you have applied to.</p>
      ...
      <div className="grid2">
        <section className="card">
          <h2>Compose</h2>
          {recipients.length === 0 ? (
            <p className="muted">No available recipients yet.</p>
          ) : (
            <form method="POST" action="/api/messages/send" className="form">
              <input type="hidden" name="csrfToken" value={session.csrfToken} />
              <input type="hidden" name="returnTo" value="/talent/messages" />
